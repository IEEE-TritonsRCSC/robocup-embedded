...
default PID_Outputs::PID_Outputs() {
	this->pout = 0;
	this->iout = 0;
	this->dout = 0;
}
	...
	void
	PID_Outputs::setPout(float pout)
{
	this->pout = pout;
}
...
PID_Object::PID_Object(float maxOutput, float integralLimit, float deadBand, float target, PID_Gains gains) : maxOutput(maxOutput), integralLimit(integralLimit), deadBand(deadBand), target(target) {
	this->gains = gains;
