...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
uint8_t UART2_rxBuffer[11] = {0};
uint8_t UART2_rxCompleteFlag = 0; // Flag to indicate data reception
/* USER CODE END PD */
...
int main(void)
{
  ...
  HAL_NVIC_SetPriority(USART2_IRQn, 0, 0);  // Set priority
  ...
  while (1)
  {
	  ...
	  // Green LED blinking: STM32 receiving UART interrupt data (for testing)

	  if (UART2_rxCompleteFlag) {

		  // Reset flag
		  UART2_rxCompleteFlag = 0;
